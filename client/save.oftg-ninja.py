...
def cases_edit():
    ...
    if 'edit' in request.form:
        ...
        for key in request.form:
            if "|" in key:
                plugin, property = key.split('|', 1)
                if plugin == 'files':
                    if not 'payloads' in caselibrary.cases[casename]:
                        caselibrary.cases[casename]['payloads'] = {}
                    tempcase['payloads'][property] = caselibrary.cases[casename]['payloads'][property]
                    tempcase['plugins'][plugin][property] = request.form[key]
                elif plugin == 'configuration':
                    if not 'configuration' in caselibrary.cases[casename]:
                        caselibrary.cases[casename]['configuration'] = {}
                    if not property in caselibrary.cases[casename]['configuration']:
                        caselibrary.cases[casename]['configuration'][property] = {}
                    print caselibrary.cases[casename]['configuration'][property]
                    tempcase['configuration'][property] = request.form[key] #This appears to be a typo: caselibrary.cases[casename]['configuration'][property]
                else:
        ...
        for plugin in tempcase['plugins'].keys():
            if 'enable' in tempcase['plugins'][plugin]:
                print tempcase['plugins'][plugin]['enable']
...
def client_start():
    ...
    try:
        ...
        clientclass = Client(plugins, caselibrary.cases[case], getlocaladdr(), target, QueueHandler, loggerqueue, None)
